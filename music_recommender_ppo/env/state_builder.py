...
class StateBuilder:
    ...
    def build_state(
        ...
        """
        user_history: deque of track_ids
        context: dict with keys like time_of_day, device, location
        """
        ...
        for track_id in list(user_history)[-self.history_length:]:
            if track_id in self.song_features_df.index:
                song_vectors.append(
                    self.song_features_df.loc[track_id][self.feature_cols].values
                )
            else:
                song_vectors.append(np.zeros(len(self.feature_cols)))

        # Padding if history is short
        while len(song_vectors) < self.history_length:
            song_vectors.insert(0, np.zeros(len(self.feature_cols)))

        song_state = np.concatenate(song_vectors)

        # Context features (simple numeric encoding for now)
        ...
        return np.concatenate([song_state, context_vector]).astype(np.float32)
